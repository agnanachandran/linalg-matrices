...
public class Matrix {
	...
	private static final String NON_SQUARE_ERROR = "Error! Non-square matrix";
	...
	public static Matrix addOrSubtract(Matrix a, Matrix b, boolean isAddition) {
		...
		Matrix resultMatrix = new Matrix(a.rows, a.columns, result);
		return resultMatrix;
	}
	...
	public static double det(Matrix a) {
		...
		if (a.rows != a.columns) {
			// error
			throw new IllegalArgumentException(NON_SQUARE_ERROR);
		}
	...
	private static double detOfArray(double[][] matrix) {
		...
		if (matrix[0].length == 1) {
			...
			for (int i = 0; i < matrix.length; i++) {
				det += Math.pow(-1, i) * detOfArray(subArray(matrix, 0, i, matrix.length))
						* matrix[0][i];
			}
		}
		return det;
