	...
	public final void initUI() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				while (!(nameInput.length() < 10 && !map.containsKey(nameInput))) {
					if (nameInput.equals(Integer.toString(JOptionPane.CLOSED_OPTION))
							|| nameInput.equals(Integer.toString(JOptionPane.CANCEL_OPTION))) {
				...
				map.put(name, new Matrix(rows, columns, getContents(matElements, rows, columns)));
			}
		...
		editButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (matrices.size() == 0) {
					...
					Matrix[] matList = (Matrix[])matrices.toArray();
					...
					okButton.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent arg0) {
							...
							map.put(chosenMatrix,
									new Matrix(rows, columns, getContents(matElements, rows,
											columns)));
						}
					});
		...
		delButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (matrices.size() == 0) {
					...
					final JLabel viewLabel = new JLabel("Choose a matrix to delete", null,
							JLabel.CENTER);
					frame.add(viewLabel, BorderLayout.CENTER);
					...
					public void actionPerformed(ActionEvent arg0) {
						if (matrices.size() == 0) {
							...
							if (map.get(addOne).getRows() == map.get(addTwo).getRows()
									&& map.get(addOne).getColumns() == map.get(addTwo).getColumns()) {

								area.append("Adding matrices " + addOne + " and " + addTwo + "\n");
				...
				okButton.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent arg0) {
						if (matrices.size() == 0) {
							...
							if (map.get(subtractOne).getRows() == map.get(subtractTwo).getRows()
									&& map.get(subtractOne).getColumns() == map.get(subtractTwo)
											.getColumns()) {

								area.append("Subtracting matrices " + subtractOne + " and "
										+ subtractTwo + "\n");
								printMatrixString(subtractOne);
								area.append("\n - \n\n");
								printMatrixString(subtractTwo);
								area.append("\n = \n");
								result = Matrix.subtract(map.get(subtractOne), map.get(subtractTwo));
				...
				okButton.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent arg0) {
						if (matrices.size() == 0) {
							...
							if (map.get(multOne).getColumns() == map.get(multTwo).getRows()) {

								area.append("Multiplying matrices " + multOne + " and " + multTwo
										+ "\n");
								printMatrixString(multOne);
								area.append("\n * \n");
								printMatrixString(multTwo);
								area.append("\n = \n");
								result = Matrix.multiply(map.get(multOne), map.get(multTwo));
								printMatrix(result);
								area.append("\n");
							}
		...
		transposeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				final JLabel rrefLabel = new JLabel("Choose a matrix to transpose!", null,
						JLabel.CENTER);
				frame.add(rrefLabel);
		...
		detButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				final JLabel addLabel = new JLabel(
						"Choose a square matrix whose determinant you want to find!", null,
						JLabel.CENTER);
	...
	private double[][] getContents(String elements, int rows, int columns) {
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < columns; j++) {
				contents[i][j] = Double.parseDouble(elements.split("\\s+")[columns * i + j]);
			}
		}
		return contents;
	}
